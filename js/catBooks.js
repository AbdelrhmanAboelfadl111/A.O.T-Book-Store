...
let catBooks = document.querySelector("#BooksCat .container .row");

categories.forEach((cat) => {
  if (catBooks) {
    catBooks.innerHTML += ` <div class="col-md-3 column">
                    <div class="item w-100">
                        <a href="books.html?category=${cat.id}">
                            <div class="box box2" style="background-image: url('${cat.img}')">
                                <div class="shape w-100 h-100">
                                    <img src="imgs/catBooks/wired-outline-112-book-morph-open.gif">
                                </div>
                                <div class="bodyText w-100">
                                    <h2 class="font1 color1">${cat.name}</h2>
                                </div>
                            </div>
                        </a>
                    </div>
...
window.addEventListener("load", function () {
  setTimeout(function () {
    ...
    document?.querySelector("#BooksCat").classList.add("active");
  }, 500);
});
let currentScroll = 0;
